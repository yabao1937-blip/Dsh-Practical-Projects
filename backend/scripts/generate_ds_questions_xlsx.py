...
REPLIES = {
    ...
    "Q6": ("B", "已按 B 落地：方向只在页面展示观察，不接入密度建议；并新增「最近判断 涨✔跌✘」清单"
                "（近 20 次逐条）+ 页面上明确写「当前只展示、不参与密度建议」"),
...
# 方向"新鲜度"对照：同一个近期窗口，训练截止不同 → 最近 20 次命中差别很大
dir_fresh = direction_report(coarse, split_ts="2026-09-04")
dir_stale = direction_report(coarse, split_ts="2026-08-23")
q3_rows = []
...
sheet(
    ...
    [
        ...
        ["★你的答复：B（2026-09-24）",
         "已按 B 落地，并且把“观察”做成看得见的东西：粗精煤泥页「向前验证」区块新增一行"
         "「最近判断：涨✔ 跌✔ 涨✘ …（近20次命中 0.xxx）」，鼠标悬停能看到每次的时间、预测与实际；"
         "同一区块里明确写「当前只展示、不参与密度建议」。方向怎么算、检验窗口是什么，都写在同页说明里。"],
        ["什么时候再回来看",
         "建议积累 1~2 个月（约 30~60 个新采样点）后复盘：看「最近判断」的实际命中是否还在 0.7 附近。"
         "若明显下滑，说明机理变了（原煤/工艺变化），要重算；若稳定，再来定“接入操作提示”怎么接。"],
        ["★观察时先留意这一条（实测）",
         "方向的命中率跟【模型新鲜度】关系很大：同一个 9 月窗口，训练截止 9-03 的模型最近 20 次命中 %.3f，"
         "而训练截止 8-22 的（更旧的）模型最近 20 次只有 %.3f（接近瞎猜），整体命中也由 %.3f 降到 %.3f。"
         "→ 建议每拿到一批新数据就重训一次（Q4 选的是“全部数据”，正好适合这个节奏）；"
         "页面上「向前验证」会显示它是“训练→哪一天”，可以直接对着看。"
         % (dir_fresh["recentHit"], dir_stale["recentHit"], dir_fresh["hit"], dir_stale["hit"])],
        ["接入前必须先定的两件事",
         "① 方向偏涨时，密度到底动不动（默认不动？还是按现在那套 Δρ=min(|ΔA|/15, 0.03) 规则动）；"
         "② 动多少 —— 方向没有幅度信息，只能给“一个步长”，要现场先认可那个步长。"],
        ["你要怎么回答",
...
sheet(
    ...
    [
        ["已收到的答复", "Q1 不做（维持 10 因子）｜Q2 A（下一条采样点）｜Q3 A（打赢取均值/在线平滑）｜"
                        "Q4 C（全部数据）｜Q6 B（方向只展示观察）｜Q7 A（固定目录供给）；Q5/Q8 待确定"],
        ["需要你继续回的", "① Q5：采样后约几小时能拿到化验结果；② Q8：有没有在线或每班 1 次的灰分数据"],
